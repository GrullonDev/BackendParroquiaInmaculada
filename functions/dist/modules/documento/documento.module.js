"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DocumentoModule = void 0;
var common_1 = require("@nestjs/common");
var documento_service_1 = require("./documento.service");
var documento_resolver_1 = require("./documento.resolver");
var typeorm_1 = require("@nestjs/typeorm");
var documento_entity_1 = require("./entity/documento.entity");
var cliente_entity_1 = require("../cliente/entity/cliente.entity");
var sacerdote_entity_1 = require("../sacerdote/entity/sacerdote.entity");
var DocumentoModule = (function () {
    function DocumentoModule() {
    }
    DocumentoModule = __decorate([
        (0, common_1.Module)({
            imports: [typeorm_1.TypeOrmModule.forFeature([documento_entity_1.Documento, cliente_entity_1.Cliente, sacerdote_entity_1.Sacerdote])],
            providers: [documento_service_1.DocumentoService, documento_resolver_1.DocumentoResolver],
        })
    ], DocumentoModule);
    return DocumentoModule;
}());
exports.DocumentoModule = DocumentoModule;
//# sourceMappingURL=documento.module.js.map